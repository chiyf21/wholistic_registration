...
def synquant_org(input, minSZ, smoothingfactor, Mu, Sigma):
    ...
    for i in range(maxThres, minThres - 1, -stepThres):
        ...
        maskroiIDx = [
            idx for idx in maskroiIDx if len(idx) >= 10
        ]  # Remove regions smaller than 10 pixels

        # Process each valid region
        for j in range(len(maskroiIDx)):
            idxtmp = maskroiIDx[j]
            N = len(idxtmp)
            if minSZ < N and N < 10000:
                idxtmpneiL1 = regionGrowxx_3D(
                    idxtmp, smoothingfactor, lenx, leny, lenz, xxshift1, yyshift1
                )
