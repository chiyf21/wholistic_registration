...
def FindInitPhase(
    ...
    """
    Build an initial phase for a single moving slice by:
    1) splitting the moving slice into overlapping patches,
    2) searching the best-matching reference slice for each patch
       within the same XY region,
    3) fusing patch-wise best z values into a dense z-map,
    4) smoothing the z-map globally.

    Parameters
    ----------
    data_mov : cp.ndarray or np.ndarray
        Moving slice, expected shape (H, W) or (H, W, 1).
    data_ref : cp.ndarray or np.ndarray
        Reference volume, expected shape (H, W, Z).
    z_ratio : float
        Reserved for compatibility with your coordinate convention.
        This function currently returns phase[..., 2] in reference z-index space.
    patch_size : int
        Patch size along both Y and X.
    overlap : float, default=0.5
        Patch overlap ratio in [0, 1).
    smooth_sigma : float, default=20.0
        Gaussian smoothing sigma for the fused dense z-map.
    use_gradient : bool, default=True
        If True, match on gradient magnitude instead of raw intensity.
    use_confidence : bool, default=True
        If True, multiply patch vote weights by a confidence term (top1 - top2).
    confidence_eps : float, default=1e-3
        Lower bound for confidence weight.
    ...
    Returns
    -------
    phase_init : cp.ndarray
        Initial phase with shape (H, W, 1, 3), matching data_mov.shape[:3].
    z_map_smooth : cp.ndarray
    ...
    if use_gradient:
        mov_feat = calculate.grad_mag_2d(mov2d)  # (H, W)

        # Apply 2D gradient magnitude slice by slice.
        # Using stack here avoids explicit Python loops in the main logic.
        ref_feat = cp.stack(
    ...
    # Ensure the last patch touches the image boundary
    ...
    # ------------------------------------------------------------------
    # Extract patch tensors with sliding_window_view
    #
    # mov_windows shape:
    #   (H-P+1, W-P+1, P, P)
    #
    # ref_windows shape after sliding on YX:
    #   expected to contain patch windows for every z slice
    # ------------------------------------------------------------------
    ...
    # Select only patch starts in ys/xs
    # mov_patches: (Ny, Nx, P, P)
    mov_patches = mov_windows[ys[:, None], xs[None, :], :, :]

    # Depending on CuPy version, ref_windows may be:
    #   (H-P+1, W-P+1, Z, P, P)
    # or already similar.
    # After indexing:
    #   (Ny, Nx, Z, P, P)
    ref_patches = ref_windows[ys[:, None], xs[None, :], ...]
    ...
    # Make sure layout is (Ny, Nx, Z, P, P)
    # If the last two dims are not patch dims, adjust here if needed.
    # In the common case no transpose is needed.
    ...
    # ------------------------------------------------------------------
    # Weighted ZNCC, fully vectorized across:
    #   patch_y, patch_x, candidate_z
    # ------------------------------------------------------------------
    weight_patch = calculate.hann2d(P, P).astype(cp.float32)  # (P, P)
    weight_patch = weight_patch[None, None, None, :, :]  # (1,1,1,P,P)

    # Expand moving patches to compare with all Z candidates
    # mov_patches_exp: (Ny, Nx, 1, P, P)
    mov_patches_exp = mov_patches[:, :, None, :, :]
    ...
    numerator = cp.sum(weight_patch * mov_centered * ref_centered, axis=(-2, -1))  # (Ny, Nx, Z)

    mov_denom = cp.sum(weight_patch * mov_centered * mov_centered, axis=(-2, -1))   # (Ny, Nx, 1)
    ref_denom = cp.sum(weight_patch * ref_centered * ref_centered, axis=(-2, -1))   # (Ny, Nx, Z)

    scores = numerator / (cp.sqrt(mov_denom * ref_denom) + weight_eps)               # (Ny, Nx, Z)

    # ------------------------------------------------------------------
    # Best z and optional confidence
    # ------------------------------------------------------------------
    best_z = cp.argmax(scores, axis=2).astype(cp.float32)          # (Ny, Nx)
    best_score = cp.max(scores, axis=2)                            # (Ny, Nx)

    if use_confidence and Z >= 2:
        top2 = cp.partition(scores, kth=Z - 2, axis=2)[:, :, -2]
        confidence = cp.maximum(best_score - top2, confidence_eps)  # (Ny, Nx)
    else:
        confidence = cp.ones_like(best_score, dtype=cp.float32)

    # ------------------------------------------------------------------
    # Dense fusion by overlap-weighted voting
    # No Python loops; use cp.add.at on flattened accumulators.
    # ------------------------------------------------------------------
    yy_local = cp.arange(P, dtype=cp.int32)
    xx_local = cp.arange(P, dtype=cp.int32)

    # Global indices for every patch pixel
    # y_idx, x_idx: (Ny, Nx, P, P)
    y_idx = ys[:, None, None, None] + yy_local[None, None, :, None]
    x_idx = xs[None, :, None, None] + xx_local[None, None, None, :]

    # Broadcast to full patch grid
    ...
    # Patch vote weights:
    #   Hann window * patch confidence
    vote_weight = calculate.hann2d(P, P).astype(cp.float32)[None, None, :, :]  # (1,1,P,P)
    vote_weight = vote_weight * confidence[:, :, None, None]            # (Ny,Nx,P,P)

    z_vote = vote_weight * best_z[:, :, None, None]                     # (Ny,Nx,P,P)
    ...
    # Fill any uncovered pixels with the global weighted average
    ...
    # ------------------------------------------------------------------
    # Build phase_init
    # phase_init.shape[:3] must match data_mov.shape[:3]
    # Here output is (H, W, 1, 3)
    # ------------------------------------------------------------------
    Y, X = cp.meshgrid(
    ...
    return phase_init.get() if isinstance(phase_init,cp.ndarray) else phase_init, z_map_smooth.get() if isinstance(z_map_smooth,cp.ndarray) else z_map_smooth
    ...
    for layer in range(layer_num, -1, -1):
        ...
        for iter in range(iterNum):
            ...
            if iter == iterNum - 1:
                ...
                # Update error hist2ory (shift and add new error)
