...
def _get_layer_shape_and_spacing(SZ, SZ_HR, layer, zRatio_raw, zRatio_HR, pyramid_mode="xy_only"):
    ...
    if pyramid_mode == "xy_only":
        x = max(1, int(round(SZ[0] / scale_xy)))
        y = max(1, int(round(SZ[1] / scale_xy)))
        z = SZ[2]

        x_hr = max(1, int(round(SZ_HR[0] / scale_xy)))
        y_hr = max(1, int(round(SZ_HR[1] / scale_xy)))
        z_hr = SZ_HR[2]

        zRatio = zRatio_raw / scale_xy
        zRatio_hr = zRatio_HR / scale_xy

        scale_z_mov = 1
        scale_z_ref = 1

    elif pyramid_mode == "isotropic":
        x = max(1, int(round(SZ[0] / scale_xy)))
        y = max(1, int(round(SZ[1] / scale_xy)))
        ...
        # 目标：让当前层 z spacing 尽量接近 xy spacing
        scale_z_mov = max(1, int(round(scale_xy / zRatio_raw)))
        scale_z_ref = max(1, int(round(scale_xy / zRatio_HR)))

        z = max(1, int(round(SZ[2] / scale_z_mov)))
        z_hr = max(1, int(round(SZ_HR[2] / scale_z_ref)))

        zRatio = zRatio_raw * scale_z_mov
        zRatio_hr = zRatio_HR * scale_z_ref

    else:
        raise ValueError(f"Unknown pyramid_mode: {pyramid_mode}")

    return {
        "x": x, "y": y, "z": z,
        "x_hr": x_hr, "y_hr": y_hr, "z_hr": z_hr,
        "zRatio": zRatio, "zRatio_hr": zRatio_hr,
    }
...
def build_reference_trap_mask_from_bad_moving(
    ...
    gz = cupy_ndimage.sobel(ref_sm, axis=2) / 8.0
...
def initialize_phase_for_layer(option, SZ,layer, x, y, z, zRatio, zRatio_hr):
    ...
    if "phase" in option and option["phase"] is not None:
        ...
        phase_current = cp.stack([X, Y, Z * zRatio_hr / zRatio], axis=-1).astype(cp.float32)
